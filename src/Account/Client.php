 ...
 *
 * Mirrors the core-account surface of the Go SDK (account.go): login (with or
 * without 2FA), API-key login, OTP validation, ping, OTP generate/verify/
 * disable and registration.
 *
 * Unlike the Go SDK, individual calls never repeat status/error checking: every
 * operation delegates to a single OpHandler that holds the operation -> status
 * error maps (matching the Go SDK's httpErrorMessages) and maps non-success
 * status codes to typed PortalException subclasses (UnauthorizedException,
...
{
    /**
     * Default endpoint for the account service (Go SDK DefaultEndpoint with an
     * https scheme).
     */
    public const DEFAULT_ENDPOINT = 'https://account.pinner.xyz';

    /**
     * Operation IDs — kept index-aligned with the Go SDK's OpLogin iota block
     * so the shared operation/error infrastructure stays recognizable.
     */
     ...
     * prefix), matching the Go SDK's PostApiAuthKeyWithResponse.
     *
     * @throws PortalException on a failed/error response (mapped by OpHandler).
     */
     ...
     * Location header (mirrors Go's extractTokenFromRedirect).
     */
     ...
     * OpHandler, matching the Go SDK's operationString and httpErrorMessages.
     */
