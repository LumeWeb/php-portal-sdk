 ...
 *
 * Mirrors the Go SDK's internal/http error factories (AuthError, PlainError,
 * NotFoundError, ForbiddenError, BadRequestError, ConflictError). A factory is
 * stored in an OpHandler operation map and, on make(), produces a typed
 * PortalException subclass carrying the HTTP status and raw response body.
 *
 * plainError() intentionally returns the base PortalException (no sentinel
 * wrapping), matching the Go SDK where plain errors are not errors.Is-matched
 * against a sentinel.
 */
