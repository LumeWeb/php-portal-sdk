 ...
 *
 * Unlike the Go SDK, which returns plain error wrappers from every call, the
 * PHP port centralizes status/error checking in OpHandler. Every non-success
 * HTTP response surfaces as a PortalException (or a typed subclass) carrying
 * the HTTP status code and the raw response body, so callers can branch on the
