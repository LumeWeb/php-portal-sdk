...
PHP SDK for the LumeWeb Portal **account** (core auth) and **admin** (quota)
APIs.

Client code is generated from the vendored OpenAPI specs
(`openapitools/openapi-generator`) and wrapped behind a thin handwritten layer
that **centralizes status/error handling**, so individual calls never repeat
per-call status checks.
...
src/                        Handwritten SDK layer (namespace LumeWeb\Portal\)
  ...
  Http/
    OpHandler.php           Centralized status/error handling
...
`LumeWeb\Portal\Http\OpHandler::handleResponse()`:
...
The operation → status error maps used by `OpHandler` are:
